...
export class DeliveryPersonRepository implements IDeliveryPersonRepository {
  ...
  async createdDeliveryPerson(
    data: DeliveryPersonDto,
    ...
    try {
      ...
      throw new BadRequestException(
        `error: deliveryPersonRepository: ${error.message}`,
      );
    }
  }
  async updateDeliveryPerson(
    ...
    try {
      ...
      throw new BadRequestException(
        `error: updateDeliveryPerson: ${err.message}`,
      );
    }
  }
  async getAllDeliveryPerson(tenantId: string): Promise<DeliveryPerson[]> {
    try {
      ...
      throw new BadRequestException(
        `une erreur lors de requête ${error.message}`,
      );
    }
  }
