...
export class PaginationCustomerUseCase{
    ...
    async execute(limit:number, page:number):Promise<PaginatedResponseRepository<Customer>>{
        try {
            return this.customerRepository.paginate(limit, page)
        } catch (error) {
            this.logger.error(`Failled to retrieve customer, (limit: ${limit}, page: ${page}) ${error.stack}`)
            throw new BadRequestException('Failled to retrieve customer', error.message);
        }
    }
}